...
import { formatDateSummary, formatPrice } from "./utils";
...
export function BookingSummary({
  ...
  const hasFlatEveningSurcharge =
    selectedService.title === "Location Du Studio" || selectedService.id === "prestation-photographe";
    ...
    <div className="rounded-2xl border border-white/10 bg-background/70 p-5">
      ...
      <div className="mt-4 space-y-3 text-sm text-white/65">
        <p className="flex justify-between gap-4">
          ...
          <strong className="text-right text-white">{serviceLabel ?? selectedService.title}</strong>
        </p>
        ...
        {!isQuoteOnly && (
          <p className="flex justify-between gap-4">
            <span>Durée</span>
            <strong className="text-right text-white">{durationLabel}</strong>
          </p>
        )}
        ...
        {!isQuoteOnly && selectedNightHours > 0 && (
          <p className="flex justify-between gap-4">
        ...
        <div className="border-t border-white/10 pt-3 mt-3">
          {isQuoteOnly ? (
            <p className="flex justify-between gap-4 items-center">
              <span>Tarif</span>
              <strong className="text-right text-xl text-secondaire">Sur devis</strong>
            </p>
          ) : (
            <>
              <p className="flex justify-between gap-4 items-center">
