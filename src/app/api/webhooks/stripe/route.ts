...
export async function POST(req: Request) {
  ...
  if (event.type === 'checkout.session.completed') {
    ...
    // 1. Récupérer et valider les métadonnées de réservation
    const metadata = session.metadata;
    if (!metadata) {
      console.error('Aucune métadonnée trouvée pour la session:', session.id);
    ...
    if (!email || !date || !slot || !service) {
      console.error('Métadonnées incomplètes pour la session:', session.id);
      return NextResponse.json({ error: 'Métadonnées de réservation incomplètes' }, { status: 400 });
    }

    // 2. Double validation du prix payé
    let expectedAmount;
    try {
      const booking = parseBookingMetadata(metadata);
      expectedAmount = calculateExpectedPaymentAmount(booking);
    } catch (metadataError) {
      console.error('Métadonnées de réservation invalides:', metadataError);
      return NextResponse.json({ error: 'Métadonnées invalides' }, { status: 400 });
    }

    if (session.amount_total !== expectedAmount.amountInCents) {
      console.error('Montant Stripe inattendu', {
        sessionId: session.id,
        expected: expectedAmount.amountInCents,
        actual: session.amount_total,
      });
      return NextResponse.json({ error: 'Montant de paiement incohérent' }, { status: 400 });
    }
    ...
    const safe = {
      ...
      formattedDate: escapeHtml(formattedDate || date),
      slot: escapeHtml(slot),
      service: escapeHtml(service),
      message: escapeHtml(message || 'Aucune information complémentaire'),
    };
    ...
    try {
      ...
      latestCharge = paymentIntent?.latest_charge as Stripe.Charge | null;
    } catch (retrieveError) {
      console.error("Erreur lors de la récupération des détails de session Stripe :", retrieveError);
    }

    const finalReceiptUrl = safeHttpsUrl(
    ...
    if (siret) {
      try {
        ...
        if (govRes.ok) {
          ...
          if (govData.results && govData.results.length > 0) {
            companyAddress = govData.results[0].siege?.adresse || 'Adresse introuvable';
          }
        }
      } catch (err) {
      ...
      companyBlockAdminHtml = `
        ...
        <p style="margin: 0 0 15px 0; font-weight: bold;">${safe.companyName}</p>
        <p style="margin: 15px 0 5px 0; font-size: 14px; color: ${lightTextColor};">SIRET</p>
        <p style="margin: 0 0 15px 0; font-weight: bold;">${safe.siret}</p>
        <p style="margin: 15px 0 5px 0; font-size: 14px; color: ${lightTextColor};">Localisation</p>
        <p style="margin: 0 0 15px 0; font-weight: bold;">${escapeHtml(companyAddress)}</p>
      `;
    }
    ...
    try {
      ...
      const isDuplicate =
        calendarError?.status === 409 ||
        calendarError?.code === 409 ||
        String(calendarError?.message).includes('already exists') ||
