    ...
    <main className="min-h-screen bg-background px-6 py-12 text-white md:px-10">
      <div className="mx-auto max-w-7xl">
        <header className="mb-8 flex flex-wrap items-end justify-between gap-5">
          ...
          <div className="flex gap-3"><button onClick={sendTestEmail} disabled={isSendingTest} className="rounded-full border border-secondaire/40 bg-secondaire/10 px-4 py-2 text-xs font-black uppercase tracking-widest text-secondaire">{isSendingTest ? "Envoi..." : "Tester Mailjet"}</button><button onClick={async () => { await fetch("/api/admin/logout", { method: "POST" }); router.push("/admin/login"); }} className="rounded-full border border-white/10 bg-white/[0.06] px-4 py-2 text-xs font-black uppercase tracking-widest text-white/70">Déconnexion</button></div>
        </header>
